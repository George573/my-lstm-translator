...
datasets = {}
for path in ['en-fr-13-len-727-sampled.csv', 'data/raw/fra.txt']:
    pairs = list(dict.fromkeys(p for p in iter_parallel_rows(path) if parallel_partition(*p, validation_fraction=.01, test_fraction=.01, seed=42) == 'test'))
    datasets[path] = random.Random(42).sample(pairs, min(300, len(pairs)))
...
for name in ['model-256-6.latest.pth', 'model-32mil-param.latest.pth']:
    bundle = load_checkpoint('artifacts/checkpoints/' + name)
    state = bundle.training_state
    result = {'parameters':sum(p.numel() for p in bundle.model.parameters()), 'global_step':state['global_step'], 'history':state['history'], 'teacher_forcing_ratio':state['teacher_forcing_ratio'], 'datasets':{}}
